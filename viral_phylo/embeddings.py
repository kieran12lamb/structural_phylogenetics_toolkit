...
# Prevent libomp crash on macOS when multiple libraries link OpenMP
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

import numpy as np


from viral_phylo.binaries import find_torch_python
...
def compute_umap_projection(
    ...
    """Compute 2D UMAP projection coordinates from protein embeddings."""
    try:
        import umap
        n_samples = len(taxa_names)
        if n_samples < 3:
            return {name: [float(i * 2.0 - 1.0), 0.0] for i, name in enumerate(taxa_names)}
        actual_neighbors = max(2, min(n_neighbors, n_samples - 1))
        umap_metric = metric.lower().strip()
        if umap_metric in ("l1", "manhattan", "cityblock"):
            umap_metric = "manhattan"
        elif umap_metric not in ("cosine", "euclidean"):
            umap_metric = "cosine"
        ...
        return {name: [round(float(coords[i, 0]), 4), round(float(coords[i, 1]), 4)] for i, name in enumerate(taxa_names)}
    except Exception as e:
        print(f"[PLM Warning] UMAP projection calculation failed: {e}")
        return {}
...
def run_embedding_pipeline(
    ...
    if compute_umap:
        try:
            umap_coords = compute_umap_projection(embeddings, taxa_names, metric=metric)
            if umap_coords:
                umap_path = os.path.join(output_dir, f"{prefix}_umap_{model}.json")
                ...
                print(f"[PLM] 2D UMAP projection coordinates saved to: '{umap_path}'")
        except Exception as e:
            print(f"[PLM Warning] Could not compute 2D UMAP coordinates: {e}")
    ...
    np.savez_compressed(
        ...
        umap=json.dumps(umap_coords) if umap_coords else ""
    )
...
def main():
    ...
    parser.add_argument(
        "--no-umap",
        action="store_true",
        help="Disable automatic 2D UMAP projection computation."
    )
